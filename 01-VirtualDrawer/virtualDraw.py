...
myColors = [[5, 110, 0, 19, 255, 255],  # orange
...
def findClor(img, myColors, myColorVal):
...
def getContour(img):
    contours, hieraarchy = cv2.findContours(
...
def drawOnCanvas(myPoints, myColorVal):
    for point in myPoints:
        cv2.circle(imgResult, (point[0], point[1]),
                   10, myColorVal[point[2]], cv2.FILLED)


while cap.isOpened():
    ret, img = cap.read()
    imgResult = img.copy()
    newPoints = findClor(img, myColors, myColorVal)
    if len(newPoints) != 0:
